...
Test PatchGuard with APGD L2 attack.

Usage:
    python test_apgd_l2.py --class_name bottle --dataset mvtec --dataset_dir ./datasets/mvtec --epsilon 3.0 --checkpoint_dir ./checkpoints/
"""
...
from apgd_l2_patchguard import apgd_l2_attack_patchguard


def test_apgd_l2(model, test_loader, device, args, epsilon, n_iter, n_restarts):
    """Evaluate model under APGD L2 attack."""
    model.eval()
    ...
    for batch in test_loader:
        ...
        with torch.set_grad_enabled(True):
            adv_images = apgd_l2_attack_patchguard(
                model=model,
                images=images,
                masks=patch_masks,
                epsilon=epsilon,
...
def run_test_apgd_l2(args):
    ...
    # APGD L2 evaluation for each epsilon
    for epsilon in args.epsilon_l2:
        image_auc, pixel_auc = test_apgd_l2(
            model, test_loader, device, args, epsilon, args.n_iter_l2, args.n_restarts_l2
        )
        display_results(
            {"Image-level AUC": image_auc, "Pixel-level AUC": pixel_auc},
            f"APGD-L2 attack (eps={epsilon}, n_iter={args.n_iter_l2})"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test PatchGuard with APGD L2 attack")
    parser.add_argument("--class_name", type=str, required=True)
    parser.add_argument("--dataset", type=str, default="mvtec")
    parser.add_argument("--dataset_dir", type=str, default="./datasets/mvtec")
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints/")
    parser.add_argument("--epsilon_l2", type=float, nargs='+', default=[3.0])
    parser.add_argument("--n_iter_l2", type=int, default=200)
    parser.add_argument("--n_restarts_l2", type=int, default=1)
