...
Test PatchGuard with PGD L0 attack.

Usage:
    python test_pgd_l0.py --class_name bottle --dataset mvtec --dataset_dir ./datasets/mvtec --sparsity 20 --checkpoint_dir ./checkpoints/

    # Multiple sparsity values:
    python test_pgd_l0.py --class_name bottle --sparsity 10 20 50 100 --checkpoint_dir ./checkpoints/
"""
...
from pgd_l0_patchguard import pgd_l0_attack_patchguard


def test_pgd_l0(model, test_loader, device, args, sparsity, num_steps, step_size, n_restarts):
    """Evaluate model under PGD L0 attack."""
    model.eval()
    ...
    for batch in test_loader:
        ...
        with torch.set_grad_enabled(True):
            adv_images = pgd_l0_attack_patchguard(
                model=model,
                images=images,
                masks=patch_masks,
                sparsity=sparsity,
...
def run_test_pgd_l0(args):
    ...
    # PGD L0 evaluation for each sparsity value
    for sparsity in args.sparsity:
        image_auc, pixel_auc = test_pgd_l0(
            model, test_loader, device, args,
            int(sparsity), args.num_steps_l0, args.step_size_l0, args.n_restarts_l0
        )
        display_results(
            {"Image-level AUC": image_auc, "Pixel-level AUC": pixel_auc},
            f"PGD-L0 attack (sparsity={int(sparsity)}, steps={args.num_steps_l0})"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test PatchGuard with PGD L0 attack")
    parser.add_argument("--class_name", type=str, required=True)
    parser.add_argument("--dataset", type=str, default="mvtec")
    parser.add_argument("--dataset_dir", type=str, default="./datasets/mvtec")
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints/")
    parser.add_argument("--sparsity", type=float, nargs='+', default=[20.0])
    parser.add_argument("--num_steps_l0", type=int, default=30)
    parser.add_argument("--step_size_l0", type=float, default=20.0)
    parser.add_argument("--n_restarts_l0", type=int, default=10)
