...
Test PatchGuard with APGD L∞ attack.

Usage:
    python test_apgd_linf.py --class_name bottle --epsilon_linf 8 --checkpoint_dir ./checkpoints_adv/
    python test_apgd_linf.py --class_name bottle --epsilon_linf 4 8 16 --checkpoint_dir ./checkpoints_adv/
"""
...
from apgd_linf_patchguard import apgd_linf_attack_patchguard


def test_apgd_linf(model, test_loader, device, args, epsilon, n_iter, n_restarts):
    """Evaluate model under APGD L∞ attack."""
    model.eval()
    ...
    for batch in test_loader:
        ...
        with torch.set_grad_enabled(True):
            adv_images = apgd_linf_attack_patchguard(
                model=model,
                images=images,
                masks=patch_masks,
                epsilon=epsilon,
...
def run_test_apgd_linf(args):
    ...
    # APGD L∞ evaluation for each epsilon
    for epsilon in args.epsilon_linf:
        eps_normalized = epsilon / 255.0
        image_auc, pixel_auc = test_apgd_linf(
            model, test_loader, device, args, eps_normalized, args.n_iter_linf, args.n_restarts_linf
        )
        display_results(
            {"Image-level AUC": image_auc, "Pixel-level AUC": pixel_auc},
            f"APGD-Linf attack (eps={epsilon}/255, n_iter={args.n_iter_linf})"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test PatchGuard with APGD Linf attack")
    parser.add_argument("--class_name", type=str, required=True)
    parser.add_argument("--dataset", type=str, default="mvtec")
    parser.add_argument("--dataset_dir", type=str, default="./datasets/mvtec")
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints_adv/")
    parser.add_argument("--epsilon_linf", type=float, nargs='+', default=[8.0])
    parser.add_argument("--n_iter_linf", type=int, default=200)
    parser.add_argument("--n_restarts_linf", type=int, default=1)
